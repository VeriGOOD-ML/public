...
import polymath.ploymath as pm
...
class VerilogGenerateFlexBitwidth(Pass):
    ...
    def create_verilog_embedded(self, param, verilog_path):
        verilog = ""
        ...
        self.verilog_code = verilog
        self.write_verilog_to_file(verilog_path)
...
@register_pass
class VerilogCreateGraphBitwidth(Pass):
    ...
    def __init__(self, param):
        ...
        self.output=[]
        self.verilog_code=""
        self.variable_list=param["variable_list"]
        self.verilog_param = {"WeightN": 0, "ActivationN": 0, "BiasN": 0,  "OutputN": 0, "RateN":0,\
                              "OutputBitwidth": {"activation":0,"weight":0,"bias":0,"bits":0},\
                              "WeightBitwidth": param["weight_bitwidth"], "BiasBitwidth": param["bias_bitwidth"],\
                              "ActivationBitwidth": param["activation_bitwidth"],"algo":param["algo"],}
        with open(param["non_embedded_template_path"], 'r') as file:
            self.non_embedded_temp = file.read()
        with open(param["embedded_template_path"], 'r') as file:
            self.embedded_temp = file.read()
        # print(self.verilog_code)
        super(VerilogCreateGraphBitwidth, self).__init__()

    def create_verilog_nonembedded(self,verilog_path):
        verilog=""
        temp=self.non_embedded_temp.split("\n")
        for line in temp:
            # for param in self.verilog_param.keys():
    ...
    def apply_pass(self, node, ctx):
        ...
        #print(f"{self.counter}{node.type_modifier},{node.op_name},{node.name},{node.__class__},args:{node.args}")
        # bitwidth for input nodes
        if isinstance(node, pm.placeholder):
            #self.print_node(node)
            if (node.type_modifier=="input"):
                ...
                node.kwargs["verilog"] = "  reg  [bitwidth-1:0] _{};\n".format(node_name)
            ...
            # parameter input
                ...
                if(node.name in self.param["param_list"]):
                    node_name = self.normalize_node_name(node)
                    node.kwargs["verilog"] = "  wire  [bitwidth-1:0] _{};\n".format(node_name) + \
                                             "  assign _{}= {};\n".format(node_name, node_name)
                    node.kwargs["verilog_name"] = node_name
                elif(str(node.name).startswith("sub")):
                    node.kwargs["bitwidth"]["bits"] += self.param["param_bitwidth"]
                    node.kwargs["verilog_name"] = node.name.replace(':', '_')
            # else:
            #     print(f"{self.counter}{node.type_modifier},{node.op_name},{node.name},{node.__class__},args:{node.args}")
        ...
        # operation nodes
            ...
            if (len(node.args)==2):
                ...
                if(node.op_name=="gt"):
                    node.kwargs["bitwidth"]["bits"] = 2
                    parent0,parent1=self.args_str(node)
                    #print(parent0,parent1)
                    node.kwargs["verilog"]="  wire  [bitwidth-1:0]{};\n".format(node.kwargs["verilog_name"])
                    node.kwargs["verilog"]+="   assign {}= ({}>={})? 1:-1;\n".format(node.kwargs["verilog_name"],parent0,parent1)
                else:
                    #print(node.args[0].kwargs.keys())
                    parent0, parent1 = self.args_str(node)
                    node.kwargs["verilog"] = "  wire  [bitwidth-1:0]{};\n".format(node.kwargs["verilog_name"]) + \
                                             "  assign {} = {} {} {};\n".format(node.kwargs["verilog_name"], parent0,
                                                                                node.kwargs["op"], parent1)
    ...
    def print_node(self,node):
        ...
        if(len(node.args)==2):
            if(isinstance(node.args[0], pm.Node)):
                parent0 = node.args[0].name
                print("args0:{}".format(parent0))
            if (isinstance(node.args[1], pm.Node)):
                parent1 = node.args[1].name
                print("args1:{}".format(parent1))
        elif(len(node.args)==1 ):
            if (isinstance(node.args[0], pm.Node)):
                parent = node.args[0].name
                print("args:{}".format(parent))
        elif (len(node.args) ==0):
            if(isinstance(node,pm.temp)):
                print(node.name)
                s=node.current_value()
    ...
    def parents_str(self,node):
        if(node.args[0].type_modifier=="param"):
            if(node.op_name == "mul"):
                parent0='mu'
            else:
                parent0 = node.args[0].name.split(':')[-1]
        elif (isinstance(node.args[0], pm.Node)):
            parent0 = node.args[0].kwargs["verilog_name"]
        elif (isinstance(node.args[0], int)):
            parent0 = str(node.args[0])
        else:
        ...
        if (node.args[1].type_modifier == "param"):
            ...
            parent1 = node.args[1].kwargs["verilog_name"]
            #parent1 = node.args[1].name
        elif (isinstance(node.args[1], int)):
            parent1 = str(node.args[1])
        else:
            print("parents not foundgit merge")
        # print(parent0)
        # print(parent1)
