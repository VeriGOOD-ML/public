...
#Quantization Assumption: assuming quantization is free and happen inside the ALU before writing ofmap back to VMEM
...
def other_access_SIMDhigh(Hardware_param, LayerObj, SIMDResult_inflayer, comp_cycles_ideal):
    ...
    #for some SIMD layers (ReLU, Pool, Element-wise add of two operands, softmax)
...
def relu_cycle_model(Hardware_param, LayerObj, SIMDResult_inflayer):
    ...
    # using a common function for SIMD layers to calculate the access from pipeline registers, instruction memory, and index tables
...
def pool_access_model(Hardware_param, LayerObj, SIMDResult_inflayer):
    ...
    if (fusion_status == "NoFusion"):
        ...
        # final ofmap access: assuming quantization is free and happen inside the ALU before writing ofmap back to VMEM 
...
def pool_cycle_model(Hardware_param, LayerObj, SIMDResult_inflayer):
    ...
    # determining the number of operatios within a tile before whcih kh/kw loops come in a SIMD ALU
    ...
    ########## compute cycle model
    #of cycles to compute one tile, k operations per cycle, compute cycles do not depend on loop order or fusion
    #cycle_oneTile_cmn = (Stile_oh * Stile_ow * Stile_batch) * (DTile_oh/Stile_oh) * (DTile_ow/Stile_ow) * math.ceil(DTile_oc/Stile_oc) * (DTile_batch/Stile_batch) \
    ...
    if Layer_name == "AvgPool":
        #cy_oneTile_div = (Stile_oh * Stile_ow * Stile_batch) * (DTile_oh/Stile_oh) * (DTile_ow/Stile_ow) * math.ceil(DTile_oc/Stile_oc) * (DTile_batch/Stile_batch)
...
def softmax_access_model(Hardware_param, LayerObj, SIMDResult_inflayer):
    #data access model for softmax layer 
...
def roialignpool_access_model(Hardware_param, LayerObj, SIMDResult_inflayer):
    ...
    if (fusion_status == "NoFusion"):
        ...
        # final ofmap access: assuming quantization is free and happen inside the ALU before writing ofmap back to VMEM 
        ofmap_access_VMEM = (OW * OH * OC * RoI * Batch) * bw_ofmap   
...
def roialignpool_cycle_model(Hardware_param, LayerObj, SIMDResult_inflayer):
    ...
    # other accesses which happens at every compute cycle
    # using a common function for SIMD class layers to calculate the access from pipeline registers, instruction memory, and index tables
    #of total compute cycles without pipeline overhead and counting one cycle for each div. IF should happen once for each div operation, 
    #Step568:
